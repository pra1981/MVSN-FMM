...
Let $\mathbf{y}_{i}$ be the $J \times 1$ observation vector for subject $i$ such that $y_{ij}$ is the observation for subject $i$ at timepoint $j$. We assume for now that $\mathbf{y}_{i}$ is fully observed. Later, we relax this assumptin by allowing for missingness in the components of $\mathbf{y}_{i}$. The general formulation of the multivariate skew normal mixure model is as follows.
