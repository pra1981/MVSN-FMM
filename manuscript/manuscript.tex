...
\end{equation}
where $p(\boldsymbol\delta_k)$ is the prior distribution for $\boldsymbol\delta_k$, which we detail in Section 3.4 As detailed in Section 3.4, if we assume $p(\boldsymbol\delta_k)$ to be multivariate normal, then $\boldsymbol\delta_k$ has a closed-form multivariate normal full conditional distribution that can be easily embedded within our proposed Gibbs sampling routine. 
...
\begin{eqnarray}
\mathbf{y}^{miss}_i|(z_i=k,t_i,\mathbf{y}^{obs}_i)&\sim& \text{N}_{J-q_i}(\boldsymbol\zeta^{cond}_{ki},\boldsymbol\Sigma^{cond}_k),~\text{where}\nonumber\\
\boldsymbol\zeta^{cond}_{ki}&=& \boldsymbol\zeta^{miss}_{ki} + \boldsymbol\Sigma_{k12} \boldsymbol\Sigma_{k22}^{-1}(\mathbf{y}^{obs}_i - \boldsymbol\zeta^{obs}_{ki}) \label{eq:impute}\\
\boldsymbol\Sigma^{cond}_k&=& \boldsymbol\Sigma_{k11} - \boldsymbol\Sigma_{k12}\boldsymbol\Sigma_{k22}^{-1}\boldsymbol\Sigma_{k21},\nonumber \\
\boldsymbol\zeta_{ki} &=& \begin{pmatrix} \boldsymbol\zeta_{ki}^{miss} \\ \boldsymbol\zeta_{ki}^{obs} \end{pmatrix}, \ \text{and} \nonumber \\
...
\psi_k)^{-1}$ using current values of $\boldsymbol\psi_k$ and $\boldsymbol\Sigma_k$. Next, for $i = 1,...,n_k$, compute $a_i = A_k \boldsymbol\psi_k^T \boldsymbol\Sigma^{-1}_k(\mathbf{y}_i - \boldsymbol\zeta_{ik})$, where $\boldsymbol\zeta_{ik} = X_i \boldsymbol\beta_k$. Finally, update $t_i$ from $\text{N}_{[0,\infty)}(a_i, \mathbf{A}_k)$.  Repeat these updates for $k = 1,...,K$. 
...
The cluster labels $z_i$ are then updated from a $\text{Multinomial}(1,\boldsymbol\upsilon_i)$ density for $i = 1,...n$. A schematic outline of the Gibbs sampler is given in the Web Appendix.
...
Since the primary objective of our proposed model is to provide a flexible framework to account for the inherent features of infant development data such as skewness, correlation among repeated measures, and missingness, we assume the number of clusters $K$ to be specified \textit{a priori}.
