...
\begin{equation}
y_{ij} = \zeta_{ijk} + t_i \psi_{jk} + \sqrt{1-\psi^2_{jk}}\epsilon_{ijk}.
\end{equation}
...
We note that the multivariate skew normal density can be expressed more compactly in terms of the matrix skew normal (MatSN) density (Chen and Gupta 2005). In particular, we define $\mathbf{Y}_{n_k \times J}$ as the response matrix with rows $\mathbf{y}_i^T$, $(i = 1,...,n_k)$, where $n_k = \sum_{i = 1}^n \mathds{1}_{(z_i = k)}$ is the total number of observations belonging to cluster $k$ ($k = 1,...,K$). 
