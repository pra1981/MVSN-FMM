...
true_c1 <- c(beta_true_c1,alpha_true_c1,delta_true,pi_true[1])
true_c2 <- c(beta_true_c2,alpha_true_c2,delta_true,pi_true[2])
true_c3 <- c(beta_true_c3,alpha_true_c3,delta_true,pi_true[3])
est_c1 <- c(beta_est_c1,alpha_est_c1,delta_est,pi_est[1])
est_c2 <- c(beta_est_c2,alpha_est_c2,delta_est,pi_est[2])
est_c3 <- c(beta_est_c3,alpha_est_c3,delta_est,pi_est[3])

load("mcmc_draws_NOSKEW_2019-07-09/class_1_est")
load("mcmc_draws_NOSKEW_2019-07-09/class_2_est")
load("mcmc_draws_NOSKEW_2019-07-09/class_3_est")

t1 <- cbind(model_component,
            params,
            true_c1,est_c1,est_c2_noskew[-(9:18)])
colnames(t1) <- c("Component",
                  "Param.",
...
```{r}
rownames(t1) <- NULL
kable(t1, booktabs = T, escape = F,caption = "Model results for simulated data with n = 1,000, J = 4, p = 2, K = 3, r = 2. 1,000 iterations were run with a burn in of 100. Missingness mechanism was MAR and P(miss) = 0. Model results for the multivariate skew normal (MSN) and multivariate normal (MN) mixtures are presented.") %>%
  kable_styling(font_size = 7) %>%
  add_header_above(c(" " = 2,"Class 1" = 3)) %>%
  group_rows(" ",1,8) %>%
  group_rows(" ",9,12) %>%
  group_rows(" ",13,16) %>%
  group_rows(" ",17,17)
